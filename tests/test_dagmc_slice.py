...
def test_is_geometry_dagmc():
    bound_dag_univ = openmc.DAGMCUniverse(filename='two_volume_touching_edge.h5m').bounded_universe()
...
def test_get_dagmc_filepath():
    bound_dag_univ = openmc.DAGMCUniverse(filename='two_volume_touching_edge.h5m')
    my_geometry = openmc.Geometry(root=bound_dag_univ)
    assert my_geometry.get_dagmc_filepath() == Path(__file__).parent/'two_volume_touching_edge.h5m'
